...
let
  # noctalia comes from the system (core.desktop.enable) and is on PATH.
  # `noctalia msg <command>` drives the running shell; `noctalia msg
  # --help` lists the commands.
  noctalia = "noctalia";

  # Render a niri `spawn` of `noctalia msg <cmd>`: each word becomes its
  # own quoted token (niri spawn takes argv). No noctalia argument
  # contains spaces.
  msg =
    cmd:
    ''spawn "${noctalia}" "msg" ''
...
{
  ## ----- niri ----------------------------------------------------------------
  # Personal niri config; replaces the system /etc/niri/config.kdl
  # baseline. Keybind reference:
  ...
  config = lib.mkIf config.my.desktop.enable {
    ...
    xdg.configFile."niri/config.kdl".text = ''
      ...
      binds {
          ...
          // ----- Screenshots (grim/slurp/satty come from nixos-core's -----
          // ----- modules/desktop.nix via core.desktop.enable) -------------
